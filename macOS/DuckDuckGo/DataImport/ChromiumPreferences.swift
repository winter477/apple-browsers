...
struct ChromiumPreferences: Decodable {
    ...
    var isNewTabShortcutsEnabled: Bool {
        ntp?.shortcustVisible ?? true // Setting is enabled by default
    }

    enum NewTabPageShortcutStyle {
        case autoGenerated, custom
    }

    var newTabPageShortcutStyle: NewTabPageShortcutStyle {
        // Chrome uses auto-generated shortcuts by default unless custom links are defined.
        if ntp?.useMostVisitedTiles == nil && customLinks != nil {
            return .custom
        }
        return (ntp?.useMostVisitedTiles ?? true) ? .autoGenerated : .custom
    }

    var customShortcuts: [CustomLink] {
        guard let customLinks else {
            return []
        }
