...
extension InMemoryDataCache: DBPUICommunicationDelegate {
    ...
    public func getInitialScanState() async -> DBPUIInitialScanState {
        return DBPUIInitialScanState(resultsFound: [], scanProgress: DBPUIScanProgress(currentScans: 0, totalScans: 0, scannedBrokers: []))
    }

    public func getMaintananceScanState() async -> DBPUIScanAndOptOutMaintenanceState {

        let scanDate = DBPUIScanDate(date: 0, dataBrokers: [])
        return DBPUIScanAndOptOutMaintenanceState(inProgressOptOuts: [], completedOptOuts: [], scanSchedule: DBPUIScanSchedule(lastScan: scanDate, nextScan: scanDate), scanHistory: DBPUIScanHistory(sitesScanned: 0))
    }
    ...
    public func getBackgroundAgentMetadata() async -> DBPUIDebugMetadata {
        ...
        return DBPUIDebugMetadata(lastRunAppVersion: "")
    }
